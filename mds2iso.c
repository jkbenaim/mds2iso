...
struct trackmode_info_s get_info_for_trackmode(const enum trackmode_e trackmode) {
    for (size_t idx = 0; !trackmode_infos[idx].last; idx++) {
        if (trackmode == trackmode_infos[idx].trackmode) return trackmode_infos[idx];
    }
...
int main(int argc, char* argv[]) {
    ...
    if (sizeof(struct mds_s) != 0x58) errx(1, "bad size of struct mds_s");

    if (sizeof(struct session_s) != 0x18) errx(1, "bad size of struct session_s");

    if (sizeof(struct track_s) != 0x50) errx(1, "bad size of struct track_s");
    ...
    // check the man page if u didnt already know
    /*
 RETURN VALUE
...
as unsigned char) that differ in s1 and s2.
    ...
    int retval = memcmp(mds.magic, "MEDIA_DESCRIPTOR", sizeof(mds.magic));
    if (retval) {
    ...
    if (mds.version[0] > 1) {
        errx(1, "sorry, mds file version %u.%u not supported", mds.version[0], mds.version[1]);
    }
    ...
    for (unsigned tracknum = 0; tracknum < numtracks; tracknum++) {
        memcpy(&tracks[tracknum],
               mds_file.data + session.track_off + tracknum * sizeof(struct track_s),
               sizeof(struct track_s));
    ...
    if (mdfname[strlen(mdfname) - 1] != 's') errx(1, "input file name does not end with 's'");
    mdfname[strlen(mdfname) - 1] = 'f';

    struct MappedFile_s mdf_file = MappedFile_Open(mdfname, false);
    if (!mdf_file.data) err(1, "couldn't open '%s' for reading", mdfname);
    free(mdfname);
    ...
    if (ti.data_len == ti.data_stride) {
        ...
        for (size_t block = 0; block < numblocks; block++) {
            size_t sRc;
            sRc = fwrite(
                mdf_file.data + (size_t)ti.data_stride * block + (size_t)ti.data_off, (size_t)ti.data_len, 1, out);
